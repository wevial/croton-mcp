...
| `BACKUP_ID` | Approved identifier for one rollback backup directory under `$CROTON_ROOT/rollback`. |
...
3. Process the pre-deployment manifest **one target at a time**, independently.
   ...
   - *Entry says `absent`:* that target was created by G1, so remove exactly that
     one path, by explicit literal path, one at a time — for example
     `rm -i -- "$CROTON_ROOT/config/croton.json"`. Once every `absent` target is
     gone, remove the directories this deployment created with `rmdir`, which
     fails safely if anything unexpected remains.

   Never use `rm -rf`, never use a wildcard, never recurse into a directory.
   Anything absent from the manifest is out of scope and left untouched.
...
5. Bridge, its configuration, its credentials, and all other `/srv/agents`
...
- [ ] Rollback outcome: registration removed, no Croton process, files restored
      or deleted by exact path, re-verification result.
